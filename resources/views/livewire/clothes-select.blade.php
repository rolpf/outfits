<div class="w-full">
    ...
    <div class="w-full grid-cols-3 grid gap-8 py-4">
        @foreach ($clothes as $cloth)
            <div x-data="{checked: {{ in_array($cloth->id, $existingClothes) }} }" class="w-full">
                <label @click="" for="{{ sprintf('cloth%s', $cloth->id) }}" :class="{'bg-blue-500 text-white': checked, 'bg-white': !checked}" class="flex flex-col items-center px-4 py-6 bg-white text-blue rounded-lg shadow-lg tracking-wide uppercase border border-blue cursor-pointer hover:bg-blue-500 hover:text-white">
